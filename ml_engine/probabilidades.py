...
class ProbabilidadesMixin:
    def obter_voto_temporal(self, ultimas_cores):
        ...
        if margem >= 8.0: peso_voto = 2.5
        elif margem >= 5.0: peso_voto = 2.0
        elif margem >= 3.0: peso_voto = 1.0
        else:
            peso_voto = 0.0
            direcao = "NEUTRO"

        peso_voto = min(peso_voto, float(getattr(self, "temporal_config", {}).get("peso_maximo_no_juiz", 2.5)))
        return {
            "direcao": direcao, "peso": peso_voto, "total": round(amostra_total, 2),
            "regime": regime, "V": round(prob_v, 2), "P": round(prob_p, 2),
            "margem": round(margem, 2), "ordens_utilizadas": ordens
        }
    ...
    def _obter_respeito_projecao_contextual(self, numero, sub_num, sub_pol):
        ...
        if ctx and ctx.get("total", 0) >= 20:
            ...
            return {"taxa_respeito": taxa, "suporte": total_ctx, "fonte": "CONTEXTO", "chave": chave}

        return {"taxa_respeito": taxa_global, "suporte": total_global, "fonte": "GLOBAL", "chave": f"N={int(numero)}"}
    ...
    def _mapear_projecoes_globais(self, dados):
        ...
        for i in range(total_dados):
            ...
            if 1 <= num <= 7:
                ...
                if alvo_idx + 1 < total_dados:
                    caminho_tem_branco = any(dados[k]["cor"] == "B" for k in range(i + 1, alvo_idx))
                    if not caminho_tem_branco:
                        ...
                        self.estatisticas_projecoes_respeito[num]["total"] += 1

                        if resultado == "G0":
                            self.estatisticas_projecoes_globais[num]["g0"] += 1
                            self.estatisticas_projecoes_respeito[num]["respeitada_g0"] += 1
                        elif resultado == "G1":
                            self.estatisticas_projecoes_globais[num]["g1"] += 1
                            self.estatisticas_projecoes_respeito[num]["respeitada_g1"] += 1
                        else:
                            self.estatisticas_projecoes_globais[num]["falha"] += 1
                            self.estatisticas_projecoes_respeito[num]["nao_respeitada"] += 1

                        inicio_ctx = max(0, alvo_idx - 11)
                        pol_ctx = [str(d["cor"]).upper() for d in dados[inicio_ctx:alvo_idx + 1]]
                        if len(pol_ctx) >= 3:
                            chave_ctx = self._chave_respeito_projecao(num, pol_ctx)
                            self.projecoes_respeito_contextual[chave_ctx]["total"] += 1
                            if resultado == "G0": self.projecoes_respeito_contextual[chave_ctx]["respeitada_g0"] += 1
                            elif resultado == "G1": self.projecoes_respeito_contextual[chave_ctx]["respeitada_g1"] += 1
                            else: self.projecoes_respeito_contextual[chave_ctx]["nao_respeitada"] += 1
        ...
        for fim in range(11, total_dados - 2):
            ...
            for regra in regras:
                ...
                stats_regra["total"] += 1
                if direcao == "VERMELHO":
                    if c0 in ("V", "B"): stats_regra["V_g0"] += 1
                    elif c1 in ("V", "B"): stats_regra["V_g1"] += 1
                else:
                    if c0 in ("P", "B"): stats_regra["P_g0"] += 1
                    elif c1 in ("P", "B"): stats_regra["P_g1"] += 1
        ...
        total_projecoes = sum(stats["total"] for stats in self.estatisticas_projecoes_respeito.values())
        total_respeitadas = sum(stats["respeitada_g0"] + stats["respeitada_g1"] for stats in self.estatisticas_projecoes_respeito.values())
        self.projecoes_respeito_metricas = {
            "ativo": True, "leitura_bilateral_v_p": False, "metodo": "RESPEITADA_VS_NAO_RESPEITADA_ATE_G1",
            "total_contagens_mapeadas": total_projecoes, "contagens_respeitadas_ate_g1": total_respeitadas,
            "contagens_nao_respeitadas": total_projecoes - total_respeitadas,
            "taxa_respeito_g0_g1_percent": round((total_respeitadas / max(total_projecoes, 1)) * 100, 2),
    ...
    def _identificar_contexto_espelho_inversao(sub_num, sub_pol):
        if not sub_num or not sub_pol: return None
    ...
    def obter_voto_contagens_consolidado(self, sub_num, sub_pol, regras_posicionais):
        tipos_consolidados = {"COEXISTENCIA_CONTAGENS_ATIVA", "FINALIZACAO_CONJUNTA_ATIVA"}
        regras_contagens = []
        for regra in regras_posicionais or []:
            tipo = str(regra.get("tipo_regra", ""))
            if tipo.startswith("V3_ATIVADOR_") or tipo.startswith("HIERARQUIA_CONTAGEM_") or tipo in tipos_consolidados:
                regras_contagens.append(regra)

        if not regras_contagens:
            return {"ativo": False, "direcao": "NEUTRO", "peso": 0.0, "margem": 0.0, "suporte": 0, "tipos": []}

        mapa = getattr(self, "cartografia_regras_contextual", {})
        if not mapa or len(sub_num) < 12 or len(sub_pol) < 12:
            return {"ativo": False, "direcao": "NEUTRO", "peso": 0.0, "margem": 0.0, "suporte": 0, "tipos": sorted({str(r.get("tipo_regra", "")) for r in regras_contagens})}

        regras_unicas = {}
        for regra in regras_contagens:
            regras_unicas.setdefault(str(regra.get("tipo_regra", "")), regra)

        eventos_contagens = [{"tipo": tipo, "direcao": regra.get("direcao", "NEUTRO"), "familia": regra.get("familia", "CONTAGENS")} for tipo, regra in regras_unicas.items()]
        chaves_familia = self._chaves_cartografia_contextual_eventos(sub_num[-12:], sub_pol[-12:], eventos_contagens)

        leituras_por_tipo = []
        for tipo in regras_unicas:
            prefixo_tipo = f"REGRA_CTX|E={tipo}"
            chaves = [c for c in chaves_familia if c == prefixo_tipo or c.startswith(f"{prefixo_tipo}|")]
            leituras_tipo = []
            for chave in chaves:
                st = mapa.get(chave)
                if not st: continue
                suporte = int(st.get("total", 0))
                if suporte < 20: continue
                v_g0, p_g0 = float(st.get("V_g0", 0)) / suporte, float(st.get("P_g0", 0)) / suporte
                v_g01, p_g01 = (st.get("V_g0", 0) + st.get("V_g1", 0)) / suporte, (st.get("P_g0", 0) + st.get("P_g1", 0)) / suporte
                score_v, score_p = (0.70 * v_g0) + (0.30 * v_g01), (0.70 * p_g0) + (0.30 * p_g01)
                
                especificidade = 0.58
                if "|TRI=" in chave and "|PAD=" in chave and "|REG=" in chave: especificidade = 1.00
                elif "|ULT=" in chave and "|BI=" in chave: especificidade = 0.98
                elif "|REG=" in chave and "|MK=" in chave: especificidade = 0.95
                elif "|COEX=" in chave or "|CONT=" in chave: especificidade = 0.92
                elif "|TRI=" in chave or "|PAD=" in chave: especificidade = 0.88
                elif "|BI=" in chave: especificidade = 0.82
                elif "|ULT=" in chave: especificidade = 0.78
                elif "|REG=" in chave or "|MK=" in chave or "|GEO=" in chave: especificidade = 0.74
                
                peso_ctx = especificidade * (suporte / (suporte + 30.0))
            ...
            if leituras_tipo:
                ...
                leituras_por_tipo.append((score_v_tipo, score_p_tipo, suporte_tipo))

        if not leituras_por_tipo:
            return {"ativo": False, "direcao": "NEUTRO", "peso": 0.0, "margem": 0.0, "suporte": 0, "tipos": sorted(regras_unicas)}
        ...
        if margem < 0.025: direcao, peso = "NEUTRO", 0.0
        else:
            direcao = "VERMELHO" if score_v > score_p else "PRETO"
            peso = 18.0 * min(1.0, margem / 0.20) * min(1.0, suporte_medio / 80.0)

        return {"ativo": direcao in ("VERMELHO", "PRETO"), "direcao": direcao, "peso": round(peso, 4), "margem": round(margem, 6), "suporte": int(round(suporte_medio)), "score_v": round(score_v, 6), "score_p": round(score_p, 6), "tipos": sorted(regras_unicas)}

    def calcular_probabilidade_streak_empirica(self, cor, k):
        todos = (getattr(self, 'dados_longo', []) or []) + (getattr(self, 'dados_recencia', []) or [])
