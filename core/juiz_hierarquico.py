...
class JuizHierarquicoModificado:
    ...
    def _resolver_nivel_sem_votacao(itens, ia_modelo=None):
        ...
        for item in itens:
            ...
            # Recência mede autoridade atual; degradação crítica não vira direção.
            if estado_evolutivo == "DEGRADACAO_CRITICA":
                continue
            autoridade = (base * 0.40) + (autoridade_recente * 0.60)
            # MAIN 119 — a trajetória de perda reduz autoridade sem criar direção.
    ...
    def arbitrar_sinal(no_call_ativo, motivo_nc, expectations, inclinacao_num, geometria_mercado,
        ...
        # NÍVEL 8 — padrões visuais/observacionais. Saturação e exaustão são
        # apenas alertas estruturais e nunca geram reversão automática.
        ...
        # medida como uma única raiz causal para não duplicar V3/coexistência/
        # finalização como votos independentes. Quando essa raiz consolidada,
        # normalizada na mesma escala 0..1 da autoridade vencedora, contradiz uma
        # autoridade preliminar fraca com vantagem robusta, a informação corrige
        # a direção em vez de transformar automaticamente a discordância em NO CALL.
        ...
        # MAIN 115 — a autoridade vencedora precisa provar saúde no contexto
        # atual antes das travas de G2+. O contexto não inverte direção.
        ...
        # MAIN 129 — continuidade estrutural aprendida.
        # A morfologia não gera CALL e não inverte a direção oficial. Quando a
        # forma atual possui oposição robusta e amostra suficiente, protege a
        # operação com NO CALL; quando alinha, apenas registra sustentação.
        ...
        # Não existe "streak preta = continuar" nem "streak vermelha = reverter".
        # A própria cartografia histórica/recente da streak decide se há oposição
        # contextual robusta. Mesmo quando robusta, a direção oficial nunca é
        # invertida: o único efeito permitido é NO CALL.
        ...
        # As duas proteções de G2+ permanecem intactas: nunca trocam direção,
        # apenas convertem risco comprovado em NO CALL.
        if ia_modelo is not None:
