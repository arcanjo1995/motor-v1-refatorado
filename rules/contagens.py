class MotorContagensProjetivas:
    ...
    def _adicionar(lista, direcao, tipo, origem, peso="MEDIO", familia="CONTAGENS_OFICIAIS", **extras):
        item = {"direcao": direcao, "tipo_regra": tipo, "origem": origem, "peso": peso, "familia": familia}
        item.update(extras)
        lista.append(item)
    ...
    def mapear_janela(cls, sub_num, sub_pol, geometry_mercado, ia_modelo=None):
        ...
        operacionais = [c for c in ativas if (c["status"] in ("FECHADA", "VIVA") and c["expectativa_g0_idx"] <= horizonte_g1_idx and c["expectativa_g1_idx"] >= horizonte_g0_idx)]
        operacionais_ordenadas = sorted(operacionais, key=lambda c: (abs(c["expectativa_g0_idx"] - horizonte_g0_idx), c["expectativa_g0_idx"], -c["origem_idx"]))
